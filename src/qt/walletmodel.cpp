...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered btci address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 btci.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal btci:URI (btci:XyZ...?message=example)
            {
...
static void NotifyzbtciReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzbtciReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzbtciReset.connect(boost::bind(NotifyzbtciReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzbtciReset.disconnect(boost::bind(NotifyzbtciReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbtciTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
