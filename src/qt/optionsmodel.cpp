...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizebtciAmount"))
        settings.setValue("nAnonymizebtciAmount", 1000);

    nAnonymizebtciAmount = settings.value("nAnonymizebtciAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizebtciAmount"))
        SoftSetArg("-anonymizebtciamount", settings.value("nAnonymizebtciAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in btci.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizebtciAmount:
            return QVariant(nAnonymizebtciAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizebtciAmount:
            nAnonymizebtciAmount = value.toInt();
            settings.setValue("nAnonymizebtciAmount", nAnonymizebtciAmount);
            emit anonymizebtciAmountChanged(nAnonymizebtciAmount);
            break;
        case CoinControlFeatures:
