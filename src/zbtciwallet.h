...
#ifndef btci_ZbtciWALLET_H
#define btci_ZbtciWALLET_H
...
public:
    CzbtciWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZbtci(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZbtci(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //btci_ZbtciWALLET_H
