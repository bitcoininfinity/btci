...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZbtciStake(spend));
    } else {
        ...
        CbtciStake* btciInput = new CbtciStake();
        btciInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(btciInput);
    }
